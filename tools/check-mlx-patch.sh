...
# checkout.
#
#     ./tools/check-mlx-patch.sh            verify, and fail if it is missing
#     ./tools/check-mlx-patch.sh --apply    apply it, then verify
#
...
# already ships. Measured on mlx-swift 0.31.6: +14.6% on `ff.net.2` and +8.7% on
# `ff.net.0.proj` at M = 17,856, 6.11% at the render level, and bit-identical output.
#
...
# the numbers are bit-identical, and the binary is measurably slower on long clips for the rest of
...
# does not need it. Set LTX_ALLOW_UNPATCHED_MLX=1 to downgrade the failure to a
...
fail() {
    echo "" >&2
    echo "$1" >&2
    if [ -n "${LTX_ALLOW_UNPATCHED_MLX:-}" ]; then
        echo "" >&2
        echo "  continuing: LTX_ALLOW_UNPATCHED_MLX is set." >&2
        exit 0
    fi
    echo "" >&2
    echo "  fix:    ./tools/check-mlx-patch.sh --apply" >&2
    echo "  ignore: LTX_ALLOW_UNPATCHED_MLX=1 (this build will be ~measurably slower on long clips)" >&2
    exit 1
}
...
The pinned mlx-swift revision has probably moved under it — the patch was cut
against 0.31.6 (0bb916c67f4b9e5c682cbe02a42c701c93ab5021). Check Package.resolved
and re-cut the patch if upstream has changed matmul.cpp."
...
A build without it is correct and about measurably slower on long clips per block, with nothing
